...
urlpatterns = [
    ...
    path(
        "dashboard/api/bank-account",
        login_required(dashboard.BankAccountCntAPI.as_view()),
    ),
    ...
    path(
        "dashboard/api/guest-book", login_required(dashboard.GuestBookCntAPI.as_view())
    ),
    # 계좌번호 관리
    path(
        "bank-account",
        login_required(bank_account.BankAccountView.as_view()),
        name="bank_account",
    ),
    path("bank-account/api", login_required(bank_account.BankAccountAPI.as_view())),
    path(
        "bank-account/api/<int:req_id>",
        login_required(bank_account.BankAccountAPI.as_view()),
    ),
    # 시리얼 번호 관리
    ...
    path(
        "guest-book",
        login_required(guest_book.GuestBookView.as_view()),
        name="guest_book",
    ),
    path("guest-book/api", login_required(guest_book.GuestBookAPI.as_view())),
    path(
        "guest-book/api/<int:req_id>", login_required(guest_book.GuestBookAPI.as_view())
    ),
    # 로또 생성기
