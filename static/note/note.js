...
const requestUrl = '/note/api';   //API URL
...
function openModal(data = null) {
    ...
    $('#btn-ok').off('click').on('click', function () {
        if ($.trim($('#title').val()) === '') {
            showModal('경고', '제목을 입력하십시오.');
        }

        else if ($.trim($('textarea#note').val()) === '') {
            showModal('경고', '내용을 입력하십시오.');
        }
            ...
            param.append('note', $('textarea#note').val());
...
function compareData(data, param) {
    if (param.get('title') != data.title ||
        param.get('note') != data.note) {
        return false;
    }
    ...
    axios(
        ...
        .then(function (response) {
            if (response && response.status == 201) {
                ...
                let msg = '노트 추가에 실패하였습니다. ' + getHttpStatusMessage(response.status);
                toastr.error(msg);
            }
        })
        .catch(function (error) {
            let msg = '노트 추가에 실패하였습니다. ' + getHttpStatusMessage(error.response.status);
            toastr.error(msg);
        });
}
    ...
    axios(
        ...
        .then(function (response) {
            if (response && response.status == 200) {
                ...
                let msg = '노트 편집에 실패하였습니다. ' + getHttpStatusMessage(response.status);
                toastr.error(msg);
            }
        })
        .catch(function (error) {
            ...
            let msg = '노트 편집에 실패하였습니다. ' + getHttpStatusMessage(error.response.status);
            toastr.error(msg);
        });
}
...
function openDeleteModal() {
    ...
    //선택한 항목이 있을 경우
    if (idList.length > 0) {
        showConfirmModal('확인', '선택한 노트를 삭제하시겠습니까?',
            function (confirm) {
                //확인 버튼을 클릭한 경우
                if (confirm === true) {
                    //axios를 동기식으로 동작
