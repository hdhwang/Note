...
const requestUrl = '/bank-account/api';   //API URL
...
function initDataTable() {
    ...
    let table = $('#table-data').DataTable({
        ...
        columns: [                              //출력할 컬럼 정보
            ...
            { className: 'dt-center link', data: 'bank', defaultContent: '', render: $.fn.dataTable.render.text() },
            { className: 'dt-center link', data: 'account', defaultContent: '', render: $.fn.dataTable.render.text(), orderable: false },
            { className: 'dt-center link', data: 'account_holder', defaultContent: '', render: $.fn.dataTable.render.text() },
...
function openModal(data = null) {
    ...
    $('#menu-modal').on('show.bs.modal', function () {
        ...
        //편집인 경우
        if (data) {
            $('#modal-title').text('계좌번호 편집');      //타이틀 지정
        ...
        //추가인 경우
            ...
            $('#modal-title').text('계좌번호 추가');      //타이틀 지정
    ...
    $('#btn-ok').off('click').on('click', function () {
        if ($.trim($('#bank').val()) === '') {
            showModal('경고', '은행을 입력하십시오.');
        }

        else if ($.trim($('#account').val()) === '') {
            showModal('경고', '계좌번호를 입력하십시오.');
        }

        else if ($.trim($('#account-holder').val()) === '') {
            showModal('경고', '예금주를 입력하십시오.');
        }
            ...
            param.append('bank', $('#bank').val());
            param.append('account', $('#account').val());
            param.append('account_holder', $('#account-holder').val());
...
function compareData(data, param) {
    if (param.get('bank') != data.bank ||
        param.get('account') != data.account ||
        param.get('account_holder') != data.account_holder ||
    ...
    axios(
        ...
        .then(function (response) {
            if (response && response.status == 201) {
                toastr.success('계좌번호 추가에 성공하였습니다.');
                reloadTable();
            }
                ...
                let msg = '계좌번호 추가에 실패하였습니다. ' + getHttpStatusMessage(response.status);
                toastr.error(msg);
            }
        })
        .catch(function (error) {
            let msg = '계좌번호 추가에 실패하였습니다. ' + getHttpStatusMessage(error.response.status);
            toastr.error(msg);
        });
}
    ...
    axios(
        ...
        .then(function (response) {
            if (response && response.status == 204) {
                ...
                let msg = '계좌번호 편집에 실패하였습니다. ' + getHttpStatusMessage(response.status);
                toastr.error(msg);
            }
        })
        .catch(function (error) {
            ...
            let msg = '계좌번호 편집에 실패하였습니다. ' + getHttpStatusMessage(error.response.status);
            toastr.error(msg);
        });
}
...
function openDeleteModal() {
    ...
    //선택한 항목이 있을 경우
    if (idList.length > 0) {
        showConfirmModal('확인', '선택한 계좌번호를 삭제하시겠습니까?',
            function (confirm) {
                //확인 버튼을 클릭한 경우
                if (confirm === true) {
                    //axios를 동기식으로 동작
