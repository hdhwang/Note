...
const requestUrl = '/guest-book/api';   //API URL
...
function initDataTable() {
    ...
    let table = $('#table-data').DataTable({
        ...
        columns: [                              //출력할 컬럼 정보
            ...
            { className: 'dt-center link', data: 'name', defaultContent: '', render: $.fn.dataTable.render.text() },
            { className: 'dt-center link', data: 'amount', defaultContent: '', render: $.fn.dataTable.render.number(',') },
            { className: 'dt-center link', data: 'date', defaultContent: '', render: $.fn.dataTable.render.text() },
            { className: 'dt-center link', data: 'area', defaultContent: '', render: $.fn.dataTable.render.text() },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (html == 'Y') {
                        html = '<span class="badge bg-primary">참석</span>';
                    }

                    else if (html == 'N') {
                        html = '<span class="badge bg-danger">미참석</span>';
                    }

                    else {
                        html = '<span class="badge bg-warning">미정</span>';
                    }
        ...
        buttons: [
            {
                ...
                extend: 'excel',
                charset: 'UTF-8',
                text: '<i class="fa fa-download"></i> Excel 다운로드',
                filename: function () {
                    return '결혼식_방명록';
                }
            },
...
function openModal(data = null) {
    ...
    $('#menu-modal').off('show.bs.modal').on('show.bs.modal', function () {
        ...
        //편집인 경우
        if (data) {
            $('#modal-title').text('결혼식 방명록 편집');      //타이틀 지정
        ...
        //추가인 경우
            ...
            $('#modal-title').text('결혼식 방명록 추가');      //타이틀 지정
    ...
    $('#btn-ok').off('click').on('click', function () {
        if ($.trim($('#name').val()) === '') {
            ...
            param.append('amount', $('#amount').val());
            param.append('date', $('#date').val());
            param.append('area', $('#area').val());
            param.append('attend', $('#attend').val());
...
function compareData(data, param) {
    if (param.get('name') != data.name ||
        param.get('amount') != data.amount ||
        param.get('date') != data.date ||
        param.get('area') != data.area ||
        param.get('attend') != data.attend ||
    ...
    axios(
        ...
        .then(function (response) {
            if (response && response.status == 201) {
                toastr.success('결혼식 방명록 추가에 성공하였습니다.');
                reloadTable();
            }
                ...
                let msg = '결혼식 방명록 추가에 실패하였습니다. ' + getHttpStatusMessage(response.status);
                toastr.error(msg);
            }
        })
        .catch(function (error) {
            let msg = '결혼식 방명록 추가에 실패하였습니다. ' + getHttpStatusMessage(error.response.status);
            toastr.error(msg);
        });
}
    ...
    axios(
        ...
        .then(function (response) {
            if (response && response.status == 204) {
                toastr.success('결혼식 방명록 편집에 성공하였습니다.');
                reloadTable();
            }

            else {
                let msg = '결혼식 방명록 편집에 실패하였습니다. ' + getHttpStatusMessage(response.status);
                toastr.error(msg);
            }
        })
        .catch(function (error) {
            ...
            let msg = '결혼식 방명록 편집에 실패하였습니다. ' + getHttpStatusMessage(error.response.status);
            toastr.error(msg);
        });
}
...
function openDeleteModal() {
    ...
    //선택한 항목이 있을 경우
    if (idList.length > 0) {
        showConfirmModal('확인', '선택한 결혼식 방명록을 삭제하시겠습니까?',
            function (confirm) {
                //확인 버튼을 클릭한 경우
                if (confirm === true) {
                    //axios를 동기식으로 동작
    ...
    //선택한 항목이 없을 경우
        ...
        showModal('경고', '결혼식 방명록을 선택해 주십시오.');
    }
}
